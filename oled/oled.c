...
/**
  * @brief  显示浮点数
  * @param  
        @arg x: 起点坐标, y: 起点坐标, num:数值, a:整数的位数, b:小数的位数, size:字体大小
  * @retval none
  */
...
void OLED_Showfloat(uint8_t x, uint8_t y, float num, uint8_t a, uint8_t b, uint8_t size) {
    ...
    if (num < 0)
    {
        ...
        {
            OLED_ShowNum(x + (a + i) * (size / 2), y, (uint8_t)(num * 10) % 10, 1, size);
            num *= 10;
        }
    } else {
...
/**
  * @brief  显示数字
  * @param  x, y：起点坐标, num：要显示的数字, len：数字位数, size2：字体大小
  * @retval None
  */
...
/**
  * @brief  显示单个字符
  * @param  None
  * @retval x: 0~131, y: 0~63, size: 选择字体 16/12 , chr: 待写入字体
  */
...
/**
  * @brief  清除单个字符
  * @param  None
  * @retval x: 0~131, y: 0~63, size:选择字体 16/12, mode: 0,反白显示;1,正常显示
  */
...
void OLED_ClearString(uint8_t x, uint8_t y, uint8_t len, uint8_t Char_Size) {
...
/**
  * @brief  显示图像
  * @param  None
  * @retval x, y: 起点坐标, w, h: 图像宽度和高度, Image: 图像数据
  */
